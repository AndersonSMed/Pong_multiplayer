...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] PlayerPlatformNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is PlayerPlatformNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (PlayerPlatformNetworkObject.Length > 0 && PlayerPlatformNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(PlayerPlatformNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<PlayerPlatformBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiatePlayerPlatform instead, its shorter and easier to type out ;)")]
		public PlayerPlatformBehavior InstantiatePlayerPlatformNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlayerPlatformNetworkObject[index]);
			var netBehavior = go.GetComponent<PlayerPlatformBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<PlayerPlatformBehavior>().networkObject = (PlayerPlatformNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of PlayerPlatform
		/// </summary>
		/// <returns>
		/// A local instance of PlayerPlatformBehavior
		/// </returns>
		/// <param name="index">The index of the PlayerPlatform prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public PlayerPlatformBehavior InstantiatePlayerPlatform(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlayerPlatformNetworkObject[index]);
			var netBehavior = go.GetComponent<PlayerPlatformBehavior>();
